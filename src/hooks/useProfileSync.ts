  ...
  const syncProfile = useCallback(
    async (pubkey: string) => {
      try {
        ...
        console.log(
          "[ProfileSync] Profile not found on primary relay, checking fallback relays..."
        );
        ...
        for (const relayUrl of FALLBACK_RELAYS) {
          try {
            ...
            if (events.length > 0) {
              ...
              console.log(
                `[ProfileSync] Found profile on ${relayUrl}:`,
                profileEvent
              );
              ...
              console.log(
                "[ProfileSync] Successfully republished profile to primary relay"
              );
