 ...
 */
function validateFirebaseUser(firebaseUser: FirebaseUser): {
  isValid: boolean;
  error?: string;
} {
  if (!firebaseUser) {
...
export function useAccountLinking(
  ...
  const linkAccount = useCallback(
    async (firebaseUser: FirebaseUser, pubkey: string) => {
      console.log("Linking account:", { firebaseUser, pubkey });
      if (isLinking) {
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent("account-linked", {
            detail: {
              pubkey: pubkey.trim(),
