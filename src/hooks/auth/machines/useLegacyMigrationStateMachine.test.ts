...
import {
  ...
  type LegacyMigrationStateMachineDependencies,
} from "./useLegacyMigrationStateMachine";
...
describe("useLegacyMigrationStateMachine", () => {
  ...
  describe("Firebase Authentication", () => {
    ...
    it("should handle Firebase authentication errors", async () => {
      (mockDependencies.firebaseAuth as any).mockRejectedValue(
  ...
  describe("Account Linking Flow - With Existing Links", () => {
    ...
    it("should authenticate with linked Nostr account and complete", async () => {
      ...
      await act(async () => {
        await result.current.actions.authenticateWithLinkedNostr({
          method: "extension",
        });
      });

      expect(mockDependencies.authenticateNostr).toHaveBeenCalledWith(
        "extension",
        { method: "extension" }
      );
    ...
    it("should handle Nostr authentication errors for linked accounts", async () => {
      ...
      (mockDependencies.authenticateNostr as any).mockRejectedValue(
        new Error("Extension not available")
      );
      ...
      expect(
        result.current.getError("authenticateWithLinkedNostr")
      ).toBeDefined();
      expect(result.current.step).toBe("linked-nostr-auth");
    });
  });
  ...
  describe("Account Linking Flow - No Existing Links", () => {
    ...
    it("should handle account generation flow", async () => {
      ...
      expect(result.current.generatedAccount?.pubkey).toBe(
        "generated-pubkey-123"
      );
    });
    ...
    it("should handle account generation errors", async () => {
      (mockDependencies.createAccount as any).mockRejectedValue(
  ...
  describe("Complete Login Flow", () => {
    ...
    it("should handle complete login errors", async () => {
      (mockDependencies.setupAccount as any).mockRejectedValue(
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle linking API failures gracefully", async () => {
      ...
      (makeLinkAccountRequest as any).mockRejectedValue(
        new Error("Linking API failed")
      );
      (makeLinkedPubkeysRequest as any).mockResolvedValue([]);
    ...
    it("should handle Firebase token generation failures", async () => {
      ...
      (mockDependencies.firebaseAuth as any).mockResolvedValue(
        mockFirebaseUser
      );
    ...
    it("should clear previous errors when retrying operations", async () => {
      // First attempt fails
      (mockDependencies.firebaseAuth as any).mockRejectedValueOnce(
    ...
    it.skip("should handle HTTP 400 Bad Request during Firebase auth", async () => {
      ...
      expect(
        result.current.getError("authenticateWithFirebase")?.message
      ).toContain("400");
    });

    it.skip("should handle HTTP 401 Unauthorized during account linking", async () => {
      vi.doMock("@/hooks/auth/useLinkAccount", () => ({
        makeLinkAccountRequest: vi
          .fn()
          .mockRejectedValue(
            Object.assign(new Error("HTTP 401: Unauthorized - Invalid token"), {
              status: 401,
            })
          ),
      }));
      ...
      expect(result.current.getError("generateNewAccount")?.message).toContain(
        "401"
      );
    });
    ...
    it.skip("should handle HTTP 500 Internal Server Error during Nostr auth", async () => {
      ...
      expect(result.current.getError("bringOwnKeypair")?.message).toContain(
        "500"
      );
    });
  });
  ...
  describe("Authentication Method Specific Tests", () => {
    describe("Extension Authentication", () => {
      it("should handle extension authentication for linked accounts", async () => {
        vi.doMock("@/hooks/useLinkedPubkeys", () => ({
          makeLinkedPubkeysRequest: vi
            .fn()
            .mockResolvedValue([{ pubkey: "test-pubkey-123" }]),
        }));
      ...
      it("should handle extension user rejection", async () => {
        ...
        expect(
          result.current.getError("authenticateWithLinkedNostr")?.message
        ).toContain("rejected");
      });
    });
