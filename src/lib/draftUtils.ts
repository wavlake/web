...
export function createFutureTrackEvent(data: CreateDraftTrackData): NostrEvent {
  ...
  if (data.albumTitle) {
    tags.push(["album", data.albumTitle]);
  }

  if (data.trackNumber) {
    tags.push(["track", data.trackNumber.toString()]);
  }
  ...
  return {
    ...
    created_at: 0, // Will be set by signer
...
export function createFutureAlbumEvent(data: CreateDraftAlbumData): NostrEvent {
...
export async function parseDraftTrack(
  ...
  try {
    ...
    const metadata = {
      ...
      albumTitle: futureEvent.tags.find((tag: string[]) => tag[0] === "album")?.[1],
      ...
      trackNumber: futureEvent.tags.find((tag: string[]) => tag[0] === "track")?.[1] ? 
        parseInt(futureEvent.tags.find((tag: string[]) => tag[0] === "track")?.[1] || "0") : undefined,
      duration: futureEvent.tags.find((tag: string[]) => tag[0] === "duration")?.[1] ? 
        parseInt(futureEvent.tags.find((tag: string[]) => tag[0] === "duration")?.[1] || "0") : undefined,
    };
