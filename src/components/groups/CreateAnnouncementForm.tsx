...
import { Image, Loader2, Send, XCircle, Mic, Square, Megaphone } from "lucide-react";
...
export function CreateAnnouncementForm({ communityId, onAnnouncementSuccess }: CreateAnnouncementFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add megaphone emoji to content for announcements
      ...
      <CardContent className="p-3">
        <div className="flex gap-2">
          ...
          <div className="flex-1">
            <Textarea
              placeholder={`What would you like to announce to your community, ${displayName.split(' ')[0]}?`}
              value={content}
      ...
      <CardFooter className="flex justify-between items-center border-t px-3 py-2">
        <div className="flex gap-1">
          <Button variant="ghost" size="sm" className="text-muted-foreground h-8 px-2 text-xs" asChild>
            <label htmlFor="announcement-media-upload" className="cursor-pointer flex items-center">
