...
import { Music, Calendar, Clock, DollarSign, Tag, User, Hash, ExternalLink, Copy } from "lucide-react";
...
export function TrackDetailsDialog({ track, isOpen, onClose }: TrackDetailsDialogProps) {
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">Duration: {formatDuration(track.duration)}</span>
              </div>

              {track.price && track.price > 0 && (
                <div className="flex items-center gap-2">
                  <DollarSign className="w-4 h-4 text-muted-foreground" />
                  <span className="text-sm">Price: {formatCurrency(track.price)}</span>
                </div>
              )}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <Calendar className="w-4 h-4 text-muted-foreground" />
                <span className="text-sm">
                  Published: {new Date(track.created_at * 1000).toLocaleDateString()}
                </span>
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {track.coverUrl && (
                <Button variant="outline" size="sm" asChild>
                  <a href={track.coverUrl} target="_blank" rel="noopener noreferrer">
                    <ExternalLink className="w-4 h-4 mr-1" />
                    Cover Art
                  </a>
                </Button>
