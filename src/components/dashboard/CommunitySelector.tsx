...
function CommunityOption({ name, role, memberCount, image }: CommunityOptionProps) {
  return (
...
export function CommunitySelector({ showFullWidth = false, className = '' }: CommunitySelectorProps) {
  ...
  const getSelectedCommunityDisplay = () => {
    const community = communities.manageable.find(c => getCommunityId(c) === selectedCommunityId);
    if (!community) return 'Select Community';

    const name = getCommunityName(community);
    const role = community.pubkey === communities.owned.find(c => c.pubkey === community.pubkey)?.pubkey ? 'owner' : 'moderator';
        ...
        <SelectContent>
          ...
          {communities.manageable.length > 0 ? (
            ...
            <div className="px-2 py-4 text-center text-sm text-muted-foreground">
              ...
              <p className="text-xs mb-3">Create your artist page to start building your community!</p>
              <Button 
                size="sm" 
                className="w-full" 
...
export function CommunitySelector_Compact({ className = '' }: { className?: string }) {
  return <CommunitySelector showFullWidth={false} className={className} />;
}

// Full width version for headers or wide layouts
export function CommunitySelector_FullWidth({ className = '' }: { className?: string }) {
