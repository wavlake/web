...
const isModerator = event.tags.some(tag => 
...
```typescript
const { isLinked, firebaseUid, email, isLoading, error } = useAccountLinkingStatus();
```
...
- Shows AccountSwitcher when user is logged in
...
- `/src/components/auth/AccountSwitcher.tsx` - Account management UI
...
*Version: 1.2.0*
*Changes: Completed all user testing feedback items*